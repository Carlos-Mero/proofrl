...
def prepare_dataset(dataset_path):
    ...
    if dataset_path == "nproof/train.json":
        ...
        raise NotImplementedError("Unknown dataset name or path")
...
def train(ns):
    ...
    train_argv = argv[idx + 1:]

    script_args, grpo_cfg, model_cfg = ns.trl_parser.parse_args_and_config(train_argv)

    tdataset, _ = prepare_dataset()
    if ns.method == "rlvr":
...
def eval(ns):
    ...
    prompts, problems = prepare_dataset(ns.eval_dataset)
...
def build_parser():
    ...
    trl_parser = TrlParser((ScriptArguments, GRPOConfig, ModelConfig), add_help=False)
    ...
    p_train.set_defaults(trl_parser=trl_parser)
